
import useRevealOnScroll from '../hooks/useRevealOnScroll';

const steps = [
  {
    title: "Analyse & conception",
    desc: [
      "Compréhension des besoins, rédaction de cahier des charges",
      "Maquettage (Figma, UML)",
      "Choix des stacks adaptés : React, React Native, Vite, Tailwind, Bootstrap, PHP, Symfony, NestJS, PostgreSQL, MongoDB, MySQL",
      "Architecture MVC, sécurité, bonnes pratiques UI/UX et SEO"
    ],
    icon: (
      <svg width="36" height="36" fill="none" viewBox="0 0 24 24">
        <rect x="4" y="4" width="16" height="16" rx="4" fill="#38bdf8" />
        <path d="M8 8h8M8 12h8M8 16h8" stroke="#fff" strokeWidth="2" strokeLinecap="round" />
        <circle cx="8" cy="8" r="1.5" fill="#fff" />
        <circle cx="8" cy="12" r="1.5" fill="#fff" />
        <circle cx="8" cy="16" r="1.5" fill="#fff" />
      </svg>
    ),
  },
  {
    title: "Développement front-end",
    desc: [
      "React, React Native, Vite, Tailwind, Bootstrap, Chakra UI, Daisy UI",
      "Maquettage (Figma), expérience utilisateur (UX), accessibilité",
      "Création d’interfaces web et mobile modernes et responsives",
      "Formulaires sécurisés, bonnes pratiques UI/UX"
    ],
    icon: (
      <svg width="36" height="36" fill="none" viewBox="0 0 24 24">
        <rect x="4" y="4" width="16" height="16" rx="4" fill="#06b6d4" />
        <path d="M7 9h10M7 15h10" stroke="#fff" strokeWidth="2" strokeLinecap="round" />
        <circle cx="12" cy="12" r="3" fill="#fff" />
      </svg>
    ),
  },
  {
    title: "Développement back-end",
    desc: [
      "API RESTful : Node.js, Express, Symfony, PHP, NestJS",
      "Gestion CRUD, authentification, gestion des rôles, sécurité",
      "Tests unitaires (PhpUnit)",
      "Bases de données : PostgreSQL, MongoDB, MySQL",
      "Conteneurisation (Docker), bonnes pratiques DevOps"
    ],
    icon: (
      <svg width="36" height="36" fill="none" viewBox="0 0 24 24">
        <rect x="4" y="4" width="16" height="16" rx="4" fill="#6366f1" />
        <path d="M8 8h8M8 12h8M8 16h8" stroke="#fff" strokeWidth="2" strokeLinecap="round" />
        <circle cx="16" cy="12" r="2" fill="#06b6d4" />
      </svg>
    ),
  },
  {
    title: "Workflow & méthodologie",
    desc: [
      "Gestion de projet agile (Scrum, Kanban, Trello)",
      "Gitflow, code review, DevOps, CI/CD (Github Actions, Docker)",
      "Documentation technique, veille technologique",
      "Organisation, autonomie, esprit d’équipe, adaptation et communication efficace"
    ],
    icon: (
      <svg width="36" height="36" fill="none" viewBox="0 0 24 24">
        <path d="M4 4h16v16H4z" fill="#fbbf24" />
        <path d="M8 8h8M8 12h8M8 16h8" stroke="#fff" strokeWidth="2" strokeLinecap="round" />
        <circle cx="8" cy="8" r="1.5" fill="#fff" />
        <circle cx="8" cy="12" r="1.5" fill="#fff" />
        <circle cx="8" cy="16" r="1.5" fill="#fff" />
      </svg>
    ),
  },
  {
    title: "Déploiement, maintenance & évolution",
    desc: [
      "Déploiement continu (Github Actions, Docker)",
      "Monitoring, maintenance, évolutions et support",
      "Optimisation SEO, sécurité",
      "Amélioration continue, adaptation aux besoins",
      "Veille technologique constante"
    ],
    icon: (
      <svg width="36" height="36" fill="none" viewBox="0 0 24 24">
        <circle cx="12" cy="12" r="10" fill="#6366f1" />
        <path d="M12 7v5l4 2" stroke="#fff" strokeWidth="2" strokeLinecap="round" />
        <path d="M8 16h8" stroke="#fff" strokeWidth="2" strokeLinecap="round" />
      </svg>
    ),
  },
];

function Process() {
  const [ref, visible] = useRevealOnScroll<HTMLElement>();
  return (
    <section id="skills"
      ref={ref}
      className={`w-full py-16 bg-[#090025] text-white font-sans ${visible ? 'animate-slide-in' : 'opacity-0'}`}
    >
      <div className="max-w-5xl mx-auto px-6">
        <h2 className="text-3xl md:text-4xl font-bold text-center mb-10 text-blue-400">Mes Compétences</h2>
        {/* Grille responsive moderne */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 mt-8">
          {steps.map((step, idx) => (
            <div key={idx} className="bg-gradient-to-b from-[#2A0150] to-[#090025] border border-blue-900 rounded-2xl p-8 flex flex-col items-center text-center shadow-lg hover:shadow-2xl hover:-translate-y-1 transition duration-300 w-full">
              <div className="mb-4">{step.icon}</div>
              <h3 className="text-lg font-semibold mb-2 text-white">{step.title}</h3>
              {Array.isArray(step.desc) ? (
                <ul className="text-base text-gray-200 list-disc list-inside text-left mx-auto max-w-xs">
                  {step.desc.map((item, i) => (
                    <li key={i}>{item}</li>
                  ))}
                </ul>
              ) : (
                <p className="text-base text-gray-200">{step.desc}</p>
              )}
            </div>
          ))}
        </div>
      </div>
    </section>
  );
}

export default Process;